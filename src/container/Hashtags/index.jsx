...
export default function Hashtags(props) {
    ...
    useEffect(function () { 
        Fetch(`get-discounts-by-hashtag`, {
            qs: {
                page,
                per_page,
                is_web: 1,
    ...
    return (
        <div  className="container">
            ...
            <div style={{ display: [1].length ? 'block' : 'none' }}>
                <div className="justify-content" style={{ display: 'flex', justifyContent: 'space-between' }}>
                <h2 className="title">Hashtags <span className="greenColor">(+{data.total})</span></h2>
                    <p style={{ margin: '0px 0px', cursor: "pointer", textAlign: 'right' }} onClick={() => setShowMenu(showmenu < 3 ? showmenu + 1 : 1)}>
                        {showmenu === 1 && <AppsIcon />}
